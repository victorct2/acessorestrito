...
function make_datatablesTipoArquivo(){  
 ...
 function make_query_descr($descricao,$id_user)
 {
  ...
  $query = "
   SELECT tipo_arquivo.descricao, arquivo_upload.id AS id_arquivo, tipo_arquivo.id,nome_arquivo, arquivo, ativa, DATE_FORMAT(Data_cadastro,'%d/%m/%Y') as Data_cadastro, id_user, visto, id_arquivo, nome, DATE_FORMAT(datavisto,'%d/%m/%Y') as datavisto, statusArquivo FROM tipo_arquivo 
  ...
  WHERE ativa = 'S'  and statusArquivo='S'  and cooperado_arquivo.id_user in('".$this->session->userdata("idUsuario")."', 0  )
  ...
  ";
	...
	$query = "
  SELECT tipo_arquivo.descricao,tipo_arquivo.id,nome_arquivo, arquivo, ativa, DATE_FORMAT(Data_cadastro,'%d/%m/%Y') as Data_cadastro, id_user, visto, id_arquivo, nome, DATE_FORMAT(datavisto,'%d/%m/%Y') as datavisto, statusArquivo FROM tipo_arquivo 
  inner join arquivo_upload on arquivo_upload.tipo_arquivo = tipo_arquivo.id
  inner join cooperado_arquivo on cooperado_arquivo.id_arquivo = arquivo_upload.id
  left join usuarios on cooperado_arquivo.id_user = usuarios.id
  WHERE ativa = 'S' and statusArquivo='S'   and cooperado_arquivo.id_user in ('".$id_user."',0)  ";
 ...
 function fetch_data($limit, $start, $descricao,$id_user)
 {
  ...
  foreach($data->result_array() as $row) {
   ...
   $gruposArray = $this->session->userdata('grupos');
    ...
    $ocultar = '
        <!-- BOTÃO DE OCULTAR -->
        <a href="' . base_url("RestritoController/ocultar_arquivo/" . $id_arquivo) . '"
