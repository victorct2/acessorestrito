...
class PodcastController extends CI_Controller {
    ...
    public function cadastrarPodcast(){
		...
		if (count($mensagem) > 0) {
			...
			*/
			...
			$data['imagemApp'] = $novo_nome_imagem_app;
			...
			if($this->PodcastDao_model->insertPodcast($data)){
				...
				if(!empty($novo_nome_imagem_app)){
					copy('uploadImagens/arquivos/' . $novo_nome_imagem_app, 'assets/img/podcast/app/' . $novo_nome_imagem_app);
					chmod('assets/img/podcast/app/' . $novo_nome_imagem_app, 0777);
		...
		*/
		...
		if (count($mensagem) > 0) {
			...
			if(!empty($imagemApp)){
				...
				if($titulo !== $dadosAtuais[0]->titulo){
					$novo_nome_imagem_app = 'app-'.$friendly_url . '.' . end(explode(".",$dadosAtuais[0]->imagemApp));
				}else{
					$novo_nome_imagem_app = $dadosAtuais[0]->imagemApp;
				}
			...
			*/
			...
			if($this->PodcastDao_model->updatePodcast($data)){
				...
				if(!empty($imagemApp)){
					unlink('assets/img/podcast/app/'.$dadosAtuais[0]->imagemApp); 
					copy('uploadImagens/arquivos/' . $novo_nome_imagem_app, 'assets/img/podcast/app/' . $novo_nome_imagem_app);
					chmod('assets/img/podcast/app/' . $novo_nome_imagem_app, 0777);
					unlink('uploadImagens/arquivos/' . $novo_nome_imagem_app);
				}else{
					rename('assets/img/podcast/app/'.$dadosAtuais[0]->imagemApp,'assets/img/podcast/app/'.$novo_nome_imagem_app);
				}
    ...
    function excluirpodcast($id){
        ...
        if($this->PodcastDao_model->deletePodcast($id)){
           ...
           unlink('assets/img/programas/app/'.$dadosAtuais[0]->imagemApp); 			
