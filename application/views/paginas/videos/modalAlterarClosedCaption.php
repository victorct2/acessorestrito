<div class="modal-dialog modal-lg">
  <div class="modal-content">
    <form action="<?php echo base_url('VideosController/alterarClosedCaption') ?>" method="POST" id="formArquivo">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title">Alterar Closed Caption</h4>
      </div>
      <div class="modal-body">
            <div class="row">
              <div class="col-md-12">

                <input type="hidden" name="idClosedCaption" value="<?php echo $idClosedCaption ?>">
                
                <div class="form-group">
                    <label for="label">Label :</label>
                    <input name="label" type="text" class="form-control" id="label" placeholder="ex: Português, Inglês" value="<?php echo $closedCaption[0]->label ?>">
                </div>

                <div class="form-group">
                    <label for="srcLang">Lang :</label>
                    <input name="srcLang" type="text" class="form-control" id="srcLang" placeholder="ex: pt-br, en, fr" value="<?php echo $closedCaption[0]->srclang ?>">
                </div>  

                <div class="form-group">
                  <label>Default</label>
                  <select name="default" class="form-control" style="width: 100%;" >
                    <option value="S" <?php echo ($closedCaption[0]->default == 'S')? 'selected="selected"':'' ?>>SIM</option>
                    <option value="N" <?php echo ($closedCaption[0]->default == 'N')? 'selected="selected"':'' ?>>NÃO</option>
                  </select>
                </div>              
                ...
                <div class="form-group">
                  ...
                  <select name="status" class="form-control" style="width: 100%;" >
                    <option value="S" <?php echo ($closedCaption[0]->status == 'S')? 'selected="selected"':'' ?>>ATIVO</option>
                    <option value="N" <?php echo ($closedCaption[0]->status == 'N')? 'selected="selected"':'' ?>>INATIVO</option>
                  </select>
                </div>
                                
                <div id="resp"></div>
                <button type="submit" class="btn btn-warning">Alterar Closed Caption</button>
                <hr />
                </form>
