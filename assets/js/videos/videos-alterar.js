...
/*
    $("#videos").fileinput({
        ...
        allowedFileExtensions : ['mp4','mov','dif','flv','mkv','wmv','rmvb','vob','mpg','dv','mxf'],
		...
		$.each(data.response, function(key, file) {
	    	$.each(file, function(key, file) {
	        	var fnameVideo = file.caption;
	        	//out = out + '<li>' + 'Upload do arquivo # ' + (key + 1) + ' - '  +  fname + '  foi feito com sucesso.' + '</li>';
	        	$('#formVideos #resp').append('<input type="hidden" name="listVideos[]" value="'+fnameVideo+'" />');
                nomeDoVideo = fnameVideo;
                etapasConversao(fnameVideo,720);
	    	});
	    });
